...
class Settings(BaseSettings):
    ...
    sakai_base_url: str = Field(
        default="https://sakai.ug.edu.gh", description="Base URL for Sakai LMS instance"
    )
    sakai_username: str = Field(..., description="Sakai login username (student ID)")
    sakai_password: str = Field(..., description="Sakai login password")
    ...
    telegram_chat_id: str = Field(..., description="Telegram chat ID (user, group, or channel)")

    # Optional Configuration
    log_level: str = Field(default="INFO", description="Logging level")
    timezone: str = Field(default="Africa/Accra", description="Timezone for date parsing")
    current_semester: str | None = Field(
        default=None,
        description=(
            "Optional override. Leave empty to auto-detect the latest term from "
            "enrolled courses. Set to force a specific term, e.g. 'S2-2526'."
        ),
    )
    course_level_filter: int | None = Field(
        default=None,
